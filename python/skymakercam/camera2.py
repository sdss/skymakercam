...
"""
Python3 class to work with Skymaker camera, subclass of sdss-basecam.
.. module:: araviscam
...
__all__ = ['SkymakerCameraSystem', 'SkymakerCamera', 'SkymakerImageAreaMixIn']
...
class SkymakerCameraSystem(CameraSystem):
    ...
    def list_available_cameras(self):
        ...
        .. todo:: optionally implement a specific filter for Skymaker's if Basler 
        ...
        # Warning/todo: this gathers also cameras that are not of the Skymaker class,
...
class SkymakerCamera(BaseCamera):
    """ A FLIR (formerly Point Grey Research) Skymaker camera.
    ...
    async def _connect_internal(self, **kwargs):
        ...
        cs = SkymakerCameraSystem(SkymakerCamera)
...
class SkymakerImageAreaMixIn(ImageAreaMixIn):
...
async def singleFrame(exptim, name, verb=False, ip_add=None, config="../etc/cameras.yaml"):
    ...
    cs = SkymakerCameraSystem(
        SkymakerCamera, camera_config=config, verbose=verb, ip_list=ip_add)
...
# Example
#    SkymakerCam.py [-e seconds] [-v] [-c ../etc/cameras.yaml] {spec.age|spec.agw|...}
if __name__ == "__main__":
    ...
    # bsys = SkymakerCameraSystem(camera_class=SkymakerCamera)
